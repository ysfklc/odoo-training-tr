...
class CertificationReport(models.Model):
    ...
    def _select(self):
        select_str = """
            SELECT
                ...
                cs.id AS standard_id, c.expiry_status AS expiry_status,
    ...
    def _from(self):
        from_str = """
            ...
            JOIN certification_standard AS cs ON cs.id = c.standard_id
        """
        return from_str

    def _where(self):
        where_str = """
        rp.is_certification_body is True
        """
        return where_str

    def _group_by(self):
        group_by_str = """
            GROUP BY rp.id, c.entity_id, cs.id, c.expiry_status
        """
        return group_by_str
    ...
    def init(self):
        ...
        self.env.cr.execute("""
            CREATE or REPLACE VIEW %s as (%s FROM ( %s ) WHERE ( %s ) %s)""",
